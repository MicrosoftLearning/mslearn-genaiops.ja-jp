...
この演習では、Azure AI Foundry を使用してデプロイされたモデルを使用して、Visual Studio Code で Prompty を使用したプロンプト エンジニアリングについて探索します。

この演習には約 **40** 分かかります。

## シナリオ

受講生が Python でコーディングする方法を学ぶのを手助けするアプリを構築したいとします。 このアプリでは、受講生がコードを記述して評価するのを手助けする自動チューターが必要です。 ただし、チャット アプリがすべての回答を提供するだけでは望ましくありません。 どのように進めるかを受講生に考えさせるパーソナライズされたヒントを受講生が受け取れるようにする必要があります。

実験を開始するために GPT-4 モデルを選択しました。 ここで、プロンプト エンジニアリングを適用して、パーソナライズされたヒントを生成するチューターになるようにチャットの動作を操作する必要があります。

まず、Azure AI Foundry ポータルでこのモデルを操作するために必要なリソースをデプロイします。
