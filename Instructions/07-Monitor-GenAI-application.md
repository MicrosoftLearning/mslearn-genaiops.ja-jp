...
1. 左側のメニューを使用し、**[監視]** を選択します。
1. **[リソース配分状況]** を選択し、デプロイされたモデルとの対話の概要データをレビューします。

> **注**:[監視] ページの下部にある **[Azure Monitor メトリックス エクスプローラー]** を選択して、使用可能なすべてのメトリックスを完全に表示することもできます。 Azure Monitor が新しいタブで開きます。
...
- **要求の合計**:個別の推論要求の数。これは、モデルが呼び出された回数です。

> スループットを分析し、呼び出しあたりの平均コストを把握するのに役立ちます。

- **トークン呼び出しの合計**:プロンプト トークンと完了トークンを合算したものです。

> 待ち時間とコストの増加につながるので、課金とパフォーマンスの面で最も重要なメトリックです。

- **プロンプト トークン数**:モデル呼び出し全体を通して入力 (送信したプロンプト) で使用されたトークンの合計数です。

> これは、モデルに*質問するコスト*と考えてください。

- **完了トークン数**:モデルが出力として返したトークンの数 (基本的には応答の長さ) です。

> 生成された完了トークンは、多くの場合、トークンの使用量とコストの大部分を表します。長い回答や詳細な回答の場合は、特にそうです。

### 個々のプロンプトを比較する

1. 左側のメニューを使用し、**[トレース]** を選択します。 各 **generate_completion** 生成 AI スパンを展開すると、その子スパンが表示されます。 各プロンプトは、新しいデータ行として表されます。 次の列の内容を確認し比較します。
